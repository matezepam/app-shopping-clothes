...
export function OperationsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (canModerate) {
        tasks.push(api.moderation(token).then((value) => setModeration(value.products)));
      }
  ...
  return (
    <div className="space-y-7 pb-10">
      ...
      {canCommerce ? <section className="grid gap-6 xl:grid-cols-3">
      ...
      <section className="grid gap-6 xl:grid-cols-2">
        {canModerate ? <Panel title="Cola de moderación" subtitle="Publicaciones que requieren decisión" icon={<ShieldCheck size={20} />}>
        ...
        {canCommerce ? <Panel title="Últimos movimientos" subtitle="Trazabilidad de inventario" icon={<BarChart3 size={20} />}>
